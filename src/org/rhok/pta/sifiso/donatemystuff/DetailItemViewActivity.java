...
public class DetailItemViewActivity extends Activity {
    ...
    private static final String TAG = DetailItemViewActivity.class.getSimpleName();
    //mode  - the user is either seeing details of an offer or details of a request
	...
	private OnClickListener onSubmitRequestClick = new OnClickListener() {
		...
		public void onClick(View v) {
			Log.i(TAG, "onSubmitRequestClick - about to submit request/offer-for");
			clickCount+=1;
			if(clickCount >1){				
				//at this stage, the use has already specified the number of items required
				//check view mode
				switch(mode){
				case REQUEST:
					makeOffer(itemCount);
	...
	/**
	 * Method for converting the Donation-Offer into a Donation-Request for (THIS offer) -
	 * The user indicates how many of the offered items he is bidding for
	 */
	private void bidForDonationOffer(int itemsBiddingFor){
		
		if(offer != null){
			Log.d(TAG, "bidForDonationOffer:: About to bid for Offer "+offer.getId());
			int itemQuantity = itemsBiddingFor;
			while(itemsBiddingFor ==0 || (itemsBiddingFor > offer.getQuantity() && offer.getQuantity() >0)){
	...
	/**
	 * This is the method used to prompt the user for the number of offered items he is requesting.
	 * @return
	 */
	private int promptRequiredItems(){
		...
		View itemsPromptView = layoutInflater.inflate(R.layout.items_prompt_layout, null);
		
		AlertDialog.Builder alertDialogBuilder = new AlertDialog.Builder(context);
		// set prompts.xml to be the layout file of the alertdialog builder
		alertDialogBuilder.setView(itemsPromptView);

		final EditText input = (EditText) itemsPromptView.findViewById(R.id.userInput);
		//get the label for prompt message and change it based on the mode
		TextView promptLabel = (TextView)itemsPromptView.findViewById(R.id.tvPromptLabel);
		int label_id = (mode == OFFER? R.string.prompt_label_offer : R.string.prompt_label_request);
		...
		alertDialogBuilder
				...
				.setPositiveButton("OK", new DialogInterface.OnClickListener() {
							public void onClick(DialogInterface dialog, int id) {
								// get user's input which is the number of items required
								 itemCount = Integer.parseInt(input.getText().toString());															
							}
						})
	...
	/**
	 * This is the method for sending a donation-request in response to a donation offer
	 * @param bid - the request for donation.
	 */
	private void sendDonationOfferBid(DonationRequest bid){
		JSONObject offerJson = null;
		try {
		    String gsonJSONObject = (new Gson()).toJson(bid, DonationRequest.class);
		    if(gsonJSONObject !=null){ offerJson = new JSONObject(gsonJSONObject);}
		    //check if we were able to construct Donation-Request JSON Object
		    if(offerJson != null){ 
		    	sendVolleyRequest(DonateMyStuffGlobals.MAKE_DONATION_REQUEST_SERVLET_URL, offerJson); 
		    }
		    else{
		    	Log.w(TAG, "Unable To Construct Donation-Request JSON Object from received Donation-Request POJO");
		    	return;
		    }
		}
	...
	/**
	 * This method is used when the user is responding to a Donation-Request by making an offer
	 * @param itemsToOffer - the number of items to be offered
	 */
	private void makeOffer(int itemsToOffer){
		//for an offer to be made in response, the request object must NOT be null
		if(request != null){
			Log.d(TAG, "bidForDonationOffer:: About to bid for Offer "+offer.getId());
			...
			JSONObject offerJson = null;
			try {
			    String gsonOfferJSONObject = (new Gson()).toJson(newOffer, DonationOffer.class);
			    if(gsonOfferJSONObject !=null){ offerJson = new JSONObject(gsonOfferJSONObject);}
			    //check if we were able to construct Donation-Offer JSON Object
			    if(offerJson != null){ 
			    	sendVolleyRequest(DonateMyStuffGlobals.MAKE_DONATION_OFFER_SERVLET_URL, offerJson); 
			    }
			    else{
			    	Log.w(TAG, "Unable To Construct Donation-Offer JSON Object from received Donation-Offer POJO");
			    	return;
			    }
			}
			catch (JSONException e) {
	 ...
	 * @param url - the url of the server for either donation-requests or donation-offers
	...
	private void sendVolleyRequest(final String url,JSONObject payload){
		...
		JsonObjectRequest request = new JsonObjectRequest(
				Request.Method.POST, DonateMyStuffGlobals.MAKE_DONATION_REQUEST_SERVLET_URL,
